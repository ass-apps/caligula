                    ...
                    <div class="container-fluid mt-6 mb-6" >
                        <div class="row">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <p class="text-center">
                                        <button class="btn btn-primary" v-if="!onLoadingStore" @click="update()">Actualizar producto</button>
        ...
        methods:{
            ...
            async update(){
                ...
                try{
                    ...
                    let res = await this.$axios.post("/admin/products/update", {
                        "id": this.id,
                        "name": this.name,
            ...
            async fetch(id){
                ...
                this.id = id
                let res = await this.$axios.get("/admin/products/edit/"+id)
                this.name = res.data.product.name
                this.price = res.data.product.price
                this.mainImage = res.data.product.mainImage
                this.description = res.data.product.description
                this.categoryId = res.data.product.category_id
                this.brandId = res.data.product.brand_id
                this.imagePreview = res.data.product.mainImage
                
                res.data.product.secondary_images.forEach(data => {
                    this.secondaryImages.push({status: "listo", preview:data.image, originalName:data.image.toLowerCase(), finalName:data.image, progress:100})
                });
